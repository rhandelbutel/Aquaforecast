...
import { TrendingUp, Fish, Calendar, Target, AlertTriangle } from "lucide-react"
...
export function HarvestPredictionDashboard({
  ...
  useEffect(() => {
    ...
    const unsubSetup = GrowthService.subscribeGrowthSetup(sharedPondId, (setup) => {
      ...
      if (user?.uid && sharedPondId) void detectGrowthDueDash(sharedPondId, user.uid)
    })
  ...
  const biomassEstimate = ((currentABW || 0) * alive) / 1000
  const expectedYield =
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Estimated fish alive: {(aliveFish ?? alive).toLocaleString()}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold">
                    {expectedYield !== null ? `${expectedYield.toFixed(0)} kg` : "—"}
                  </p>
                  <p className="text-xs text-gray-500">
                    {currentABW && targetWeight
                      ? `At target ${targetWeight}g`
                      : "Set up growth tracking first"}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">
                  Progress to Target Weight ({targetWeight ?? "—"}g)
                </span>
