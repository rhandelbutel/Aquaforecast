...
function reminderDocId(isoDate: string /* YYYY-MM-DD */, time: string /* HH:mm */, userId: string) {
  ...
  try {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      for (const userPondDoc of userPondsSnap.docs) {
        ...
        // 3) Active feeding schedule(s) for the admin pond
        ...
        for (const schedDoc of schedSnap.docs) {
          ...
          for (const timeStr of feedingTimes) {
            ...
            console.log(`🕒 ${pondName} → ${timeStr} | diff = ${diffMin.toFixed(2)} mins`);

            // Only within next 60 minutes (and not in the past)
            if (diffMs <= 0 || diffMs > 60 * 60 * 1000) continue;

            // 4) Per-user reminder doc existence check
            ...
            if (remSnap.exists) {
              console.log(`⚠️ Already reminded ${userEmail} for ${pondName} at ${timeStr}`);
              continue;
            }
            ...
            // 6) Create reminder record (prevents duplicates & gives audit)
            await remRef.set({
              userId,
              email: userEmail,
              pondId: adminPondId,
