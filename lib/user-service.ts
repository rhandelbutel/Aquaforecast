...
function safeToDate(timestamp: any): Date {
  if (!timestamp) return new Date(0) // epoch sentinel to avoid “looks recent”
 ...
 */
export async function updateUserProfile(uid: string, updates: UpdatableProfile): Promise<void> {
  const userRef = doc(db, "users", uid)
  ...
  for (const [k, v] of Object.entries(cleaned)) {
    payload[k] = v === null ? deleteField() : k === "email" && typeof v === "string" ? v.toLowerCase() : v
  }

  // Merge to create-if-missing and avoid clobbering other fields
...
export function subscribeUserProfile(
  ...
  return onSnapshot(
    ...
    (err) => {
      ...
      // NOTE: we do NOT clear the UI here (don’t call cb(null))
    }
  )
}
