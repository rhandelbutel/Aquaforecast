...
export async function buildPdfFromImages(opts: {
  ...
  const addFooter = () => {
    ...
    const who =
      opts.footer?.email || opts.footer?.studentId
        ? ` • Generated by ${opts.footer?.email ?? ""}${
            opts.footer?.studentId ? ` (Student ID: ${opts.footer?.studentId})` : ""
          }`
        : "";
    pdf.setFontSize(10);
  ...
  const addPageWith = (title: string, dataUrl: string, isFirst: boolean) => {
    ...
    // Optional small title (yours are empty strings)
    if (title) {
      pdf.setFontSize(12);
...
// ---------- Single-page builder (no stretch) ----------
...
export async function buildPdfSinglePageFromImages(opts: {
  ...
  const maxW = pageW - margin * 2;
  const maxH = pageH - margin * 2 - headerH - 20; // reserve a bit for footer
  ...
  const measured = await Promise.all(
    opts.images.map(async (img) => {
      ...
      const targetH = maxW / ratio; // width-first fit
  ...
  // Optional top title (yours are empty strings, so this is invisible)
  if (opts.images[0]?.title) {
    pdf.setFontSize(12);
    pdf.text(opts.images[0].title, margin, margin + 10);
  }
